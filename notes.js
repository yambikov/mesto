...
Promise.all([api.getUserInfoApi(), api.getInitialCards()])
  ...
  1 Загружается страница в которую передаются данные с сервера (
    id карточки
    id пользователя
    массив пользователей которые лайкнули карточку
  )
  
  2 на основе этих данных создаются экземпляры класса Card в котором чекается:

  3 количество лайков (длина массива)

  4 сравнивается id пользователя с id в массиве лайков, если совпадает - карточка лайкнута и показывается активный лайк

  СОБЫТИЕ
  данные уходят на сервер
  приходит ответ с обновленными данными

  Чекается количество лайков
  лайк от текущего пользователя
